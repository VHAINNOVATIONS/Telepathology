 	...
 	*
 	* The template for this method may be found in MockFacadeRouterAsynchronousMethodImpl.ftl
 	*/
 	...
 	public void ${name}(
 		...
 		<#list parameters as parameter>
			...
			<#if "void" == declaredType>
				return;
			<#else>
	 			${declaredType} result = createInstance(${declaredType}.class);
				<#if listener??>
					asynchronouslyNotifyListener(result, ${listener}); 
				</#if>
				return;
			</#if>			
 		}
