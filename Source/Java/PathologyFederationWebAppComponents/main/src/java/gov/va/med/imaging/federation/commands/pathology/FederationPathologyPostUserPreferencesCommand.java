 ...
 *
 */
public class FederationPathologyPostUserPreferencesCommand
extends AbstractFederationPathologyCommand<Boolean, RestBooleanReturnType>
{
	 ...
	 * @param methodName
	 */
	public FederationPathologyPostUserPreferencesCommand(String routingTokenString, 
			String userId, String label, RestStringType xml,
			String interfaceVersion)
	{
		super(userId == null ? "savePathologyPreferences" : "savePathologyUserPreferences");
	...
	{
		return "to site '" + getRoutingTokenString() + "' " + ((getUserId() == null || getUserId().length() <= 0) ? "" : " for user '" + getUserId() + "'") + ", with label '" + getLabel() + "'.";
	}
