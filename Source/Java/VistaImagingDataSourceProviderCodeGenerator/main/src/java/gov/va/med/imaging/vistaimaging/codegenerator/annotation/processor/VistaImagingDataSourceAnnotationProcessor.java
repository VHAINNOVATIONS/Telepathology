 ...
 * 
 */
@SupportedAnnotationTypes("gov.va.med.imaging.vistaimaging.codegenerator.annotation.VistaImagingGeneratedDataSourceServiceType")
@SupportedSourceVersion(SourceVersion.RELEASE_6)
//@SupportedOptions("gov.va.med.imaging.mock")
public class VistaImagingDataSourceAnnotationProcessor
...
implements Processor
{
	 ...
	 * @param env
	 */
	public VistaImagingDataSourceAnnotationProcessor() 
	{
		super();
		roundIndex = 0;
	}
		...
		RoundEnvironment roundEnv)
	{
		...
		if(annotations != null && !annotations.isEmpty() && roundIndex == 0 )
		{
			...
			VistaImagingDataSourceInterfaceVisitor interfaceVisitor = 
				new VistaImagingDataSourceInterfaceVisitor();
	 ...
	 *
	 */
	class VistaImagingDataSourceInterfaceVisitor
	extends SimpleElementVisitor6<Void, Name>
	implements ElementVisitor<Void, Name>
	{
		private VistaImagingDataSourceMethodVisitor routerMethodVisitor = new VistaImagingDataSourceMethodVisitor();
		
		VistaImagingDataSourceInterfaceVisitor()
		{
			super();
		}
		...
		public Void visitType(TypeElement e, Name p)
		{
			...
			for( AnnotationMirror childElementAnnotationMirror : childElementAnnotationMirrors )
			{
				...
				if( VistaImagingGeneratedDataSourceServiceType.class.getName().equals(annotationType.toString()) )
				{
			...
			try
			{
				...
				VistaImagingDataSourceCodeGenerator codeGenerator = 
					new VistaImagingDataSourceCodeGenerator(getMessager());
				...
				*/
				...
				for(Element childElement : childElements)
				{
					if( childElement.getKind() == ElementKind.METHOD )
					{
						...
						VistaImagingDataSourceMethodCodeGenerator methodDescription = methodElement.accept(routerMethodVisitor, null);
	 ...
	 *
	 */
	class VistaImagingDataSourceMethodVisitor
	extends SimpleElementVisitor6<VistaImagingDataSourceMethodCodeGenerator, Void>
	implements ElementVisitor<VistaImagingDataSourceMethodCodeGenerator, Void>
	{
		VistaImagingDataSourceMethodVisitor()
		{
		}
		 ...
		 */
		@Override
		public VistaImagingDataSourceMethodCodeGenerator visitExecutable(ExecutableElement methodElement, Void p)
		{
			...
			VistaImagingDataSourceMethodCodeGenerator methodDescription = null;
			...
			for( AnnotationMirror childElementAnnotationMirror : childElementAnnotationMirrors )
			{
				...
				if( VistaImagingGeneratedDataSourceMethod.class.getName().equals(annotationType.toString()) )
				{
					Map<? extends ExecutableElement, ? extends AnnotationValue> annotationProperties = 
						childElementAnnotationMirror.getElementValues();
					...
					String queryFactoryMethod = null;
					String queryFactoryParameters = null;
					...
					for( ExecutableElement annotationProperty : annotationProperties.keySet() )
					{
						...
						if( "simpleName()".equals(annotationProperty.toString()) )
						{
							simpleName = annotationProperties.get(annotationProperty).toString();
							simpleName = stripSuperfluosAnnotationGarbage(simpleName);
						}
						...
						else if( "queryFactoryMethodName()".equals(annotationProperty.toString()) )
						{
							queryFactoryMethod = annotationProperties.get(annotationProperty).toString();
							queryFactoryMethod = stripSuperfluosAnnotationGarbage(queryFactoryMethod);
						}
						else if( "queryFactoryParameters()".equals(annotationProperty.toString()) )
						{
							queryFactoryParameters = annotationProperties.get(annotationProperty).toString();
							queryFactoryParameters = stripSuperfluosAnnotationGarbage(queryFactoryParameters);
						}
						...
						methodDescription = 
							new VistaImagingDataSourceMethodCodeGenerator(getMessager(), 
