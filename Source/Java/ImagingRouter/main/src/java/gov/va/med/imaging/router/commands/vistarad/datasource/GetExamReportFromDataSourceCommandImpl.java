 ...
 *
 */
public class GetExamReportFromDataSourceCommandImpl 
extends AbstractDataSourceCommandImpl<String, VistaRadDataSourceSpi> 
{
	private static final long serialVersionUID = 2743984652725810759L;
	
	private final StudyURN studyUrn;
	private static final String SPI_METHOD_NAME = "getExamReport";
	
	public GetExamReportFromDataSourceCommandImpl(StudyURN studyUrn)
	{
	...
	{
		String report = spi.getExamReport(getStudyUrn());		
		getLogger().info("Got report length of " + ((report == null || report.length() == 0) ? "0" : report.length()) + " bytes.");
		return report;
	}
