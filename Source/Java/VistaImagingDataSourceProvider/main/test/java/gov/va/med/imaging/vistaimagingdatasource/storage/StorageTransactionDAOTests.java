...
public class StorageTransactionDAOTests
{
	StorageTransactionDAO dao = new StorageTransactionDAO();
	
	@Test
	public void testTranslateCreate() throws CreationException
	{
		StorageTransaction storageTransaction = new StorageTransaction();
		Assert.assertTrue(storageTransaction.getId() == 0);

		StringBuilder builder = new StringBuilder();
		builder.append("0^^5");
		storageTransaction = dao.translateCreate(storageTransaction, builder.toString());
		
		Assert.assertTrue(storageTransaction.getId() == 5);
