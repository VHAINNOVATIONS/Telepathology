...
public class TransferStatisticsDAOTests
{
	TransferStatisticsDAO dao = new TransferStatisticsDAO();
	
	@Test
	public void testTranslateCreate() throws CreationException
	{
		TransferStatistics transferStatistics = new TransferStatistics();
		Assert.assertTrue(transferStatistics.getId() == 0);

		StringBuilder builder = new StringBuilder();
		builder.append("0^^5");
		transferStatistics = dao.translateCreate(transferStatistics, builder.toString());
		
		Assert.assertTrue(transferStatistics.getId() == 5);
